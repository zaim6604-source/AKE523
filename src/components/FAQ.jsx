...
const faqs = [
  {
    q: 'Is The Sialkot Traders a government-licensed agency?',
    a: 'Yes, we are a fully Government Licensed Overseas Employment Promoter with License No. 2196/SKT. We operate under the rules and regulations of the Government of Pakistan.',
  },
  {
    q: 'Which countries do you recruit for?',
    a: 'We recruit for Saudi Arabia, UAE, Qatar, Oman, Germany, Italy, Poland, Greece, Croatia, and more. Our network of verified employers spans the Gulf region and Europe.',
  },
  {
    q: 'Where is your office located?',
    a: 'Our office is at 11-Kashmir Centre, Kutchery Road, Sialkot, Punjab. We are conveniently located and serve clients from across the Sialkot region.',
  },
  {
...
function FAQItem({ faq, isOpen, onClick }) {
  return (
    <div className={`border rounded-2xl mb-3 overflow-hidden transition-all duration-300 ${
      isOpen ? 'border-diver-primary/30 bg-diver-primary/[0.02]' : 'border-gray-200 hover:border-diver-primary/20 bg-white'
    }`}>
      <button
        onClick={onClick}
        className="w-full flex justify-between items-center px-6 py-5 text-base font-semibold text-left cursor-pointer transition-colors hover:text-diver-primary"
        aria-expanded={isOpen}
      >
        <span className="pr-4">{faq.q}</span>
        <i className={`fas fa-chevron-down text-sm text-gray-400 transition-transform duration-300 shrink-0 ${
          isOpen ? 'rotate-180 text-diver-primary' : ''
        }`} />
      </button>
      <div
...
export default function FAQ() {
  ...
  return (
    <section id="faqs" className="section-pad bg-diver-background overflow-hidden">
