...
const countries = [
  ...
  { name: 'Croatia', city: 'Dubrovnik', jobs: 'Hospitality', img: 'fallback' },
];
...
export default function Destinations() {
  ...
  return (
    <section id="destinations" className="section-pad bg-diver-background overflow-hidden">
      <div className="container-pad">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 text-lg mt-3 max-w-2xl mx-auto">
            Trusted recruitment partnerships across the Gulf and Europe.
          </p>
        </div>
        ...
        >
          {countries.map((c, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-diver-highlight/90 via-diver-highlight/30 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-white font-bold font-heading text-lg">{c.name}</h3>
                  <span className="text-diver-accent text-xs font-medium bg-diver-highlight/40 px-2 py-0.5 rounded-full">{c.city}</span>
                </div>
                <p className="text-diver-secondary text-xs">{c.jobs}</p>
              </div>
            </div>
          ))}
        </div>
